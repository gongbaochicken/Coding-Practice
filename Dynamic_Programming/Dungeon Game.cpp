...
public:
    int calculateMinimumHP(vector<vector<int> > &dungeon) {
        int i = dungeon.size();
        int j = dungeon[0].size();
        vector<vector<int> > minhp(i+1, vector<int>(j+1, INT_MAX));  //Add dummy helper
        minhp[i][j-1] = 1;
        minhp[i-1][j] = 1;
        for(int m = i-1; m >= 0; m--){
            for(int n = j-1; n >= 0; n--){
                int bloodneed = min(minhp[m + 1][n], minhp[m][n + 1]) - dungeon[m][n];
                minhp[m][n] = (bloodneed <= 0) ? 1 : bloodneed;   
            }
        }
        return minhp[0][0];
    }
