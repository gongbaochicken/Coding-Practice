...
public:
    string minWindow(string s, string t) {
      if (s.size() == 0 || t.size() == 0) return "";
        ...
        int lowPos = 0, l = 0, r = 0;
        while(l <= r && r <= s.size()) {
            if(required) {
                table[s[r]]--;
                if(table[s[r++]] >= 0) required--;
            } else { 
                //required = 0 means we have alread reach a possible substring, which still needs to be proven
                //why not using r-l+1? previous step, r has alreday ++
                if (r - l < minLen) {
                    minLen = r -l;
                    lowPos = l;
                }
                if (table[s[l]] >= 0) required++;
                table[s[l++]]++;
            }
        }
        return minLen == INT_MAX ? "" : s.substr(lowPos, minLen);
    }
};
